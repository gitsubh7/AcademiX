...
// ───────────────────────────────────────────────
// 1. Step 1: Redirect user to Google Auth
// // ───────────────────────────────────────────────
// export const googleAuth = asyncHandler(async (req, res, next) => {
//   const auth2Client = new google.auth.OAuth2(
//     process.env.GOOGLE_CLIENT_ID,
...
// // ───────────────────────────────────────────────
// // 2. Step 2: Handle Google Redirect
// // ───────────────────────────────────────────────
// export const redirectGoogleAuth = asyncHandler(async (req, res, next) => {
//   const auth2Client = new google.auth.OAuth2(
//     process.env.GOOGLE_CLIENT_ID,
...
//   const html = `
//     <script>
//       window.opener.postMessage(${JSON.stringify(tokens)}, "${origin}");
//       window.close();
//     </script>
//   `;
//   res.setHeader("Content-Type", "text/html");
...
// // ───────────────────────────────────────────────
// // 3. Step 3: Add recurring class to Google Calendar
// // ───────────────────────────────────────────────
// export const addClass = asyncHandler(async (req, res, next) => {
//   const accessToken  = req.headers.authorization?.split(" ")[1];
...
//     // Send updated tokens if refreshed internally
//     const newTokens = auth2Client.credentials;

//     res.json({
//       message: "Class added & synced to Google Calendar",
//       tokens: newTokens,  // Frontend should update access_token if changed
//     });
//   } catch (err) {
//     console.error("Google Calendar error →", err.errors || err);
...
//         error: "Google token expired or revoked. Please reconnect.",
//       });
//     } else if (err.code === 403) {
//       return res.status(403).json({
//         error: "Calendar write scope missing. Reconnect Google Calendar.",
//       });
//     }
